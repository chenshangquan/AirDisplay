...
//{
//	CAirDispLocalDlg *pThis = (CAirDispLocalDlg*)pParam;
//	if (!pThis)
//	{
//		return -1;
//	}
...
// CAirDispLocalDlg 对话框

CAirDispLocalDlg::CAirDispLocalDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CAirDispLocalDlg::IDD, pParent)
{
...
void CAirDispLocalDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CAirDispLocalDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_COMMAND(ID_32771, &CAirDispLocalDlg::OnAbout)
	ON_COMMAND(ID_EXIT, &CAirDispLocalDlg::OnExit)
	ON_COMMAND(ID_SHOW, &CAirDispLocalDlg::OnShow)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDOK, &CAirDispLocalDlg::OnBnClickedOk)
	ON_MESSAGE(WM_TRAY_NOTIFYICON, &CAirDispLocalDlg::OnNotifyIcon)
	ON_WM_SIZE()
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	ON_COMMAND(ID_KEEP_WIDTH_HEIGHT, &CAirDispLocalDlg::OnClickMenuKeepWidthHeight)
	ON_COMMAND(ID_ABOUT, &CAirDispLocalDlg::OnClickMenuAbout)
	ON_MESSAGE(WM_OPENHIDDEV_SUCCESS, &CAirDispLocalDlg::OnOpenHidDevSuccess)
	ON_MESSAGE(WM_VOLUMECTRL_CHANGE, &CAirDispLocalDlg::OnVolumeCtrlChanged)
	ON_MESSAGE(WM_PCSLEEP_MSG, &CAirDispLocalDlg::OnPCSleepMsg)
	ON_MESSAGE(WM_PCSLEEPRESUME_MSG, &CAirDispLocalDlg::OnPCSleepResume)
	ON_MESSAGE(WM_PCREBOOTCLOSE_MSG, &CAirDispLocalDlg::OnPCRebootClose)
	ON_MESSAGE(WM_PPT_PLAY_MSG, &CAirDispLocalDlg::OnPptPlay)
	ON_MESSAGE(WM_POWER_STATUS_MSG, &CAirDispLocalDlg::OnPowerStatus)
END_MESSAGE_MAP()


// CAirDispLocalDlg 消息处理程序
/*
...
BOOL CAirDispLocalDlg::OnInitDialog()
{
...
CString CAirDispLocalDlg::GetIniFilePath()
{
    CString strIniFilePath = CLogo::GetModuleFullPath() + _T("\\AirDispLocal.ini");
...
BOOL CAirDispLocalDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CAirDispLocalDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CAirDispLocalDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CAirDispLocalDlg::OnAbout()
{
	// TODO: 在此添加命令处理程序代码
	if (m_pcVerDlg)
	{
...
void CAirDispLocalDlg::OnHide()
{
	m_pcMainDlg->ShowWindow( SW_HIDE );
	AfxGetMainWnd()->ShowWindow(SW_HIDE);
}

void CAirDispLocalDlg::OnMini()
{
	OnHide();
}

void CAirDispLocalDlg::OnBannerShow()
{
	OnHide();
	if (m_pcBannerDlg)
	{
...
void CAirDispLocalDlg::SetBannerAutoHide()
{
	SetTimer(g_nAutoHideBannerTimerID,5000,NULL); //5s自动隐藏
}

void CAirDispLocalDlg::OnBannerClose()
{
...
void CAirDispLocalDlg::OnTimer(UINT_PTR nIDEvent)   
{   
...
void CAirDispLocalDlg::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	CAboutDlg dlgAbout;
	dlgAbout.DoModal();
...
LRESULT CAirDispLocalDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	// TODO: Add your specialized code here and/or call the base class
	switch (message)
	{
...
void CAirDispLocalDlg::UdiskOut()
{	
	//销毁相关数据再退出
...
LRESULT CAirDispLocalDlg::OnNotifyIcon(WPARAM wparam, LPARAM lparam)
{
...
void CAirDispLocalDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CAirDispLocalDlg::OnDestroy()
{
	//投屏中拔掉先停止投屏
	StopProjectScreen(false);
...
void CAirDispLocalDlg::InitVideoEncoderParam(u8 byVideoType)
{
	TVideoEncParam tVideoEncParam;
	memset(&tVideoEncParam, 0, sizeof(TVideoEncParam));
...
void CAirDispLocalDlg::InitAudioEncoderParam(u8 byAudioMode, u16 wAudioDuration)
{
	m_cEncoder.SetAudioEncParam(byAudioMode,wAudioDuration);
}

void CAirDispLocalDlg::OnStartScreenCatch()
{
	m_cEncoder.StartEnc();
}

void CAirDispLocalDlg::OnStopScreenCatch()
{
	m_cEncoder.StopEnc();
}

void CAirDispLocalDlg::OnClickMenuKeepWidthHeight()
{
...
void CAirDispLocalDlg::OnClickMenuAbout()
{
	OnAbout();
}

CDataListClass* CAirDispLocalDlg::GetVideoDataList()
{
	return &m_cVideoDataList;
}

CDataListClass* CAirDispLocalDlg::GetAudioDataList()
{
	return &m_cAudioDataList;
}

void CAirDispLocalDlg::SolveReadInfo(BYTE* recvDataBuf)
{
	if (!recvDataBuf)
	{
		return;
...
//void CAirDispLocalDlg::SendCmdToHid( int nCmd )
//{
//	switch (nCmd)
//	{
...
void CAirDispLocalDlg::StartProjectScreen()
{
...
void CAirDispLocalDlg::StopProjectScreen(bool bNotifyHid)
{
	if (m_bIsProjecting)
	{
		//取消默认音频设备
...
void CAirDispLocalDlg::SolveNetStatusNty( NET_STATUS emStatus )
{
	if (m_pcMainDlg)
	{
		if (!m_bIsProjecting)
...
//HANDLE CAirDispLocalDlg::GetHidHandle(HID_TYPE emType)
//{
//	return m_HidDevice[emType].hndHidDevice;
//}
//
//THidDevice & CAirDispLocalDlg::GetHidDevie(HID_TYPE emType)
//{
//	return m_HidDevice[emType];
//}

void CAirDispLocalDlg::StartTrayIconChange()
{
	SetTimer(TrayIconChangeTimerID, 300, NULL);
}

void CAirDispLocalDlg::StopTrayIconChange()
{
...
CEncoder & CAirDispLocalDlg::GetEncode()
{
	return m_cEncoder;
}
...
void CAirDispLocalDlg::SetMouseMessage( WORD wMM,CPoint MousePosition,UINT nFlags,short zDelta )
{
...
void CAirDispLocalDlg::SetMouseKB( INPUT & KeyBoardInput,WORD wVk,bool bDown )
{
    // Set the input type
...
void CAirDispLocalDlg::GetResolution(int &nWidth, int &nHeight)
{
...
void CAirDispLocalDlg::CheckResolution()
{
    int nWid = GetSystemMetrics(SM_CXSCREEN);
    int nHei = GetSystemMetrics(SM_CYSCREEN);
...
LRESULT CAirDispLocalDlg::OnOpenHidDevSuccess( WPARAM wParam, LPARAM lParam )
{
	if (m_bHidOpen)
	{
		//注册通知窗口
...
HRESULT CAirDispLocalDlg::SetDefaultAudioPlaybackDevice( LPCWSTR devID )
{
...
void CAirDispLocalDlg::InitDefaultAudioDevice()
{
	m_bSetDefAudoDev = false;
	m_wstrSysAudioID = NULL;
...
void CAirDispLocalDlg::UnInitDedaultAudioDevice()
{
	if (!m_bSetDefAudoDev)
	{
		return;
...
void CAirDispLocalDlg::InitVolumeCtrl()
{
	if (!m_bSetDefAudoDev)
	{
		return;
...
void CAirDispLocalDlg::UninitVolumeCtrl()
{
	if (!m_bSetDefAudoDev)
	{
		return;
...
LRESULT CAirDispLocalDlg::OnVolumeCtrlChanged( WPARAM wParam, LPARAM lParam )
{
...
LRESULT CAirDispLocalDlg::OnPCSleepMsg( WPARAM wParam, LPARAM lParam )
{
	if (m_bHidOpen)
	{
		PRINTMSG("\r\n收到系统休眠或睡眠消息\r\n");
...
void CAirDispLocalDlg::SolvePCNty()
{
	//睡眠、休眠处理
	if (m_bIsSleep)
	{
...
LRESULT CAirDispLocalDlg::OnPCSleepResume( WPARAM wParam, LPARAM lParam )
{
	//睡眠、休眠处理
	if (m_bIsSleep)
	{
...
LRESULT CAirDispLocalDlg::OnPCRebootClose( WPARAM wParam, LPARAM lParam )
{
	if (m_bHidOpen)
	{
		PRINTMSG("\r\n收到系统重启、关机或注销消息\r\n");
...
bool CAirDispLocalDlg::GetSysUserName( CString &strName )
{
...
void CAirDispLocalDlg::CheckDedaultAudioDevice()
{
...
LRESULT CAirDispLocalDlg::OnPptPlay( WPARAM wParam, LPARAM lParam )
{
...
void CAirDispLocalDlg::SendPptCmdToHid( EnumPptStatus emStatus )
{
	if ( EM_PPT_PLAYING == emStatus )        //ppt正在播放
	{
		/*BYTE acBuf[LENTH_OUT_BUFFER_CMD] = {0};
...
void CAirDispLocalDlg::SolvePptNty( PPT_STATUS emPptStatus )
{
	switch ( emPptStatus )
	{
...
LRESULT CAirDispLocalDlg::OnPowerStatus( WPARAM wParam, LPARAM lParam )
{
	if ( !m_bIsProjecting )
	{
		return 0;
...
void CAirDispLocalDlg::CheckBatteryStatus()
{
...
void CAirDispLocalDlg::SetConsultVideoParam(u8 byVideoType)
{
	TVideoEncParam tVideoEncParam;
	memset(&tVideoEncParam, 0, sizeof(TVideoEncParam));
...
void CAirDispLocalDlg::MediaTypeToAudioMode( u8 byMediatype, u8& byRatio )
{
...
bool CAirDispLocalDlg::NeedCodeConsult()
{
    if (m_bNeedCodeConsult)
    {
        return true;
